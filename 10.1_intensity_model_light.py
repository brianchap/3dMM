...
def compute_strength(lmbda,fblood,fmel,vertices,triangles,s,d=0.129126):
    ...
    lit_colors = mesh.light.add_light(vertices, triangles, colors, np.array([[50,0,50]]), np.array([[2,2,2]]))
    lit_colors = lit_colors/(np.linalg.norm(lit_colors, ord=2, axis=-1, keepdims=True)+eps)

    

    normals = normals_compute(vertices,triangles)
    angles = angles_compute(normals,lit_colors)
    print("Angles Values")
    print(np.min(angles))
    print(np.max(angles))
    print(angles)

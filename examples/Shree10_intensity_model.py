...
def transform_test(vertices, obj, camera, source, h = 256, w = 256):
	...
	nm_wavelength = 550
	strength_val, angles, normals = compute_strength(nm_wavelength, 0.045, 0.22, vertices, triangles, source)
	brdf_val = []
...
# 		projected_vals = strength_val
# 		image_vals = mesh.transform.to_image(projected_vals, h, w)
		projected_vertices = transformed_vertices
		image_vertices = mesh.transform.to_image(projected_vertices, h, w)
	else:
		## world space to camera space. (Look at camera.) 
		#camera_vals = mesh.transform.lookat_camera(strength_val, camera['eye'], camera['at'], camera['up'])
	...
	#lit_colors = mesh.light.add_light(image_vertices, triangles, colors, np.array([source]), np.array([[1,1,1]]))
	#fblood, fmel = calc_mel_haem(lit_colors)
...
for i,p in enumerate(range(-200, 201, 40)):
    source = (p, 0, 200)
    #image = transform_test(vertices, obj, camera, fblood, fmel, source)
    image = transform_test(vertices, obj, camera, source)
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    #image = cv2.rotate(image,cv2.ROTATE_180)
    io.imsave('{}/0_1_{:.2f}.jpg'.format(save_folder, i), image, quality=100)
...
for i,p in enumerate(range(-200, 201, 40)):
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/0_2_{:.2f}.jpg'.format(save_folder, i), image, quality=100)
...
for i,p in enumerate(range(50, 251, 20)):
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/0_3_{:.2f}.jpg'.format(save_folder, i), image, quality=100)
...
for factor in np.arange(0.5, 1.2, 0.1):
	...
	image = image.astype(np.uint8)
	image = cv2.rotate(image,cv2.ROTATE_180)
	io.imsave('{}/4_1_{:.2f}.jpg'.format(save_folder, factor), image, quality=100)
...
for i in range(3):
	for angle in np.arange(-50, 51, 10):
		...
		image *= 255 # or any coefficient
		image = image.astype(np.uint8)
		io.imsave('{}/4_2_{}_{}.jpg'.format(save_folder, i, angle), image, quality=100)
...
for p in np.arange(-300, 301, 60): # left 0.3m -> right 0.3m
	...
	image, lc, iv = transform_test(vertices, obj, camera, source) 
	image *= 255 # or any coefficient
	image = image.astype(np.uint8)
	image = cv2.rotate(image,cv2.ROTATE_180)

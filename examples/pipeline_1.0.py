...
def transform_test(vertices, obj, camera, source, temperature, channel, h = 256, w = 256):
    ...
    return rendering, strength_val, image_vertices
...
def calc_mel_haem(colors):
    ...
    for color in colors:
        ...
        if timer % (len(colors)//3):
            print(str(100*timer/len(colors)) + "% completed processing")
        timer = timer + 1
    
    return fblood, fmel

def get_strength_metric(strengths):
    strs = np.array(strengths)
    strs = np.nan_to_num(strs)
    strs = strs/np.max(strs)
    strs = np.nan_to_num(strs)
    strength_sum = np.sum(strs)
    return int(strength_sum)

# --------- load mesh data
# --- 1. load model
# TEST matplotlib
a = [6047.140373080189, 6746.911433892884, 7576.406679021162, 8620.954442587772, 10914.742008740439, 11735.808555110054, 10277.174770365174, 8044.908168050694, 7255.014538786743, 6393.429115669975, 5747.086959072434]
plt.plot(np.arange(-200, 201, 40), a)
plt.xlabel("X-axis light source variations")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the X-axis of the light source")
...
# image, stv, vs = transform_test(vertices, obj, camera, source, temperature, 2)
...
fig_save_folder = f"{save_folder}/figs"
...
#source x-axis positions
obj['s'] = scale_init
obj['angles'] = [0, 0, 0]
obj['t'] = [0, 0, 0] 
source = (0,0,200)
temperature = 6000 # temperature is also a variable parameter (range to be determined)
channel = 0 #channel can be 0 or 1 or 2 corresponding to (R/G/B) or (B/G/R) - doubt
strengths = []
verts = []
plot_strengths = []
for i,p in enumerate(range(-200, 201, 40)):
    source = (p, 0, 200)
    image, stv, vs = transform_test(vertices, obj, camera, source, temperature, channel)
    strengths.append(stv)
    verts.append(vs)
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    #image = cv2.rotate(image,cv2.ROTATE_180)
    io.imsave('{}/0_1_{:>2d}_{:.2f}.jpg'.format(save_folder, i,p), image, quality=100)

    plot_strengths.append(get_strength_metric(stv))
print(f"Summed strength vals:{plot_strengths}")
plt.figure()
plt.plot(np.arange(-200, 201, 40), plot_strengths)
plt.xlabel("X-axis light source variations")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the X-axis of the light source")
plt.savefig(f'{fig_save_folder}/lightsource_x.png')
...
for i,p in enumerate(range(-200, 201, 40)):
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/0_2_{:>2d}_{:.2f}.jpg'.format(save_folder, i,p), image, quality=100)

    plot_strengths.append(get_strength_metric(stv))
    print(f"Summed strength vals:{plot_strengths}")
plt.figure()
plt.plot(np.arange(-200, 201, 40), plot_strengths)
plt.xlabel("Y-axis light source variations")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the Y-axis of the light source")
plt.savefig(f'{fig_save_folder}/lightsource_y.png')

plot_strengths = []
for i,p in enumerate(range(50, 251, 20)):
    source = (0,0, p)
    image, stv, vs = transform_test(vertices, obj, camera, source, temperature, channel)
    strengths.append(stv)
    verts.append(vs) 
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/0_3_{:>2d}_{:.2f}.jpg'.format(save_folder, i,p), image, quality=100)

    plot_strengths.append(get_strength_metric(stv))
    
plt.figure()
plt.plot(np.arange(50, 251, 20), plot_strengths)
plt.xlabel("Z-axis light source variations")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the Z-axis of the light source")
plt.savefig(f'{fig_save_folder}/lightsource_z.png')
...
for factor in np.arange(0.5, 1.2, 0.1):
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/1_1_{:>2d}_{:.2f}.jpg'.format(save_folder,i,factor), image, quality=100)
    i = i+1
    plot_strengths.append(get_strength_metric(stv))

plt.plot(np.arange(0.5, 1.2, 0.1), plot_strengths)
plt.xlabel("Scale values")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the scale of model")
plt.savefig(f'{fig_save_folder}/scale.png')
...
for i in range(3):
    ...
    for angle in np.arange(-50, 51, 10):
        ...
        image *= 255 # or any coefficient
        image = image.astype(np.uint8)
        io.imsave('{}/1_2_{:>2d}_{}_{}.jpg'.format(save_folder,k, i, angle), image, quality=100)
        k = k+1
        plot_strengths.append(get_strength_metric(stv))
    plt.figure()
    plt.plot(np.arange(-50, 51, 10), plot_strengths)
    plt.xlabel(f"Object's angle {i+1} values")
    plt.ylabel("Normalised strength sums")
    plt.title(f"RPPG strengths when varying angle {i+1} objects")
    plt.savefig(f'{fig_save_folder}/angle{i+1}.png')
...
for p in np.arange(500, 250-1, -40): # 0.5m->0.25m
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/2_eye_1_{:>2d}_{}.jpg'.format(save_folder,i,1000-p), image, quality=100)
    i = i+1
    plot_strengths.append(get_strength_metric(stv))
plt.figure()
plt.plot(np.arange(500, 250-1, -40), plot_strengths)
plt.xlabel("Camera position z-axis values")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the Z-axis camera position")
plt.savefig(f'{fig_save_folder}/cam_z.png')
...
for p in np.arange(-300, 301, 60): # up 0.3m -> down 0.3m
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/2_eye_2_{:>2d}_{}.jpg'.format(save_folder, i,p/6), image, quality=100)
    i = i+1
    plot_strengths.append(get_strength_metric(stv))

plt.plot(np.arange(-300, 301, 60), plot_strengths)
plt.xlabel("Camera position y-axis values")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the Y-axis camera postion")
plt.savefig(f'{fig_save_folder}/cam_y.png')
...
for p in np.arange(-300, 301, 60): # left 0.3m -> right 0.3m
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/2_eye_3_{:>2d}_{}.jpg'.format(save_folder,i, -p/6), image, quality=100)
    i = i+1
    plot_strengths.append(get_strength_metric(stv))
plt.figure()
plt.plot(np.arange(-300, 301, 60), plot_strengths)
plt.xlabel("Camera position x-axis values")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the X-axis camera position")
plt.savefig(f'{fig_save_folder}/cam_x.png')
...
for p in np.arange(-50, 51, 10):
    ...
    image *= 255 # or any coefficient
    image = image.astype(np.uint8)
    io.imsave('{}/2_eye_4_{:>2d}_{}.jpg'.format(save_folder,i, -p), image, quality=100)
    i = i+1
    plot_strengths.append(get_strength_metric(stv))
plt.figure()
plt.plot(np.arange(-50, 51, 10), plot_strengths)
plt.xlabel("Camera upwards rotation angle values")
plt.ylabel("Normalised strength sums")
plt.title("RPPG strengths when varying the upwards rotation of the camera")
plt.savefig(f'{fig_save_folder}/cam_rot_up.png')
...
strs = np.nan_to_num(strs)
strs = strs/np.max(strs)
strength_sum = np.sum(strs, axis = 1)
np.save('outputs', strs)
np.save('output_vertices',verts)


sf = 'results/intensity_normal'
if not os.path.exists(sf):
    os.mkdir(sf)
    
for k in range(len(strs)):
    img = mesh.render.render_colors(verts[k], triangles, strs[k], h, w, c=3)
    img *= 255 # or any coefficient
    img = img.astype(np.uint8)
    io.imsave('{}/image{}.jpg'.format(sf, k), img, quality=100)
    print(k)

# y = np.load('output_vertices.npy')
